...
> **串流僅支援 12 Hz tokenizer**，原因不在於缺少某個特定功能，而是解碼器的架構，這值得詳細解釋，因為它也解釋了為什麼整個管線是這樣建構的。

兩種 tokenizer 把編碼還原成音訊的方式截然不同。

**12 Hz tokenizer** 是一個自足的神經編解碼器。它的解碼器只吃一個參數（離散編碼）並直接映射成波形（`decode(audio_codes) -> wav`）。它是前饋、確定性的，且感受野是局部的：同一段編碼永遠算繪出相同的取樣，每一幀的音訊只取決於鄰近的少數幾幀。重建音色、音高與韻律所需的一切，都已內含在編碼本身。這正是串流所利用的特性，你可以取任一段*尾端視窗*的編碼、單獨解碼，新產出的取樣會與上一個視窗的算繪對齊，因此第 3 節的重疊相加交叉淡化能把連續區塊無縫接合。

**25 Hz tokenizer** 則是截然不同的設計。它的解碼器簽章是 `decode(audio_codes, xvectors, ref_mels)`，內部是一個*條件式生成模型*：一個 **DiT**（擴散變換器），以一個全域 **x-vector**（定長的語者嵌入）與取自提示語音的**參考 mel** 頻譜為條件，後接一個 **BigVGAN** 神經聲碼器，把生成出的 mel 轉成最終波形。它並非把編碼直接映射成音訊，而是透過對整個 token 序列做注意力的多步反覆去雜訊來*生成* mel，最後才聲碼化。

這個設計有兩個特性，排除了滑動視窗的做法：

- **它一次解碼整段語音，而非逐步進行。** 擴散變換器以全域、非因果的注意力對整個序列去雜訊；根本沒有「送出最後八幀」這種操作。強迫它去解碼小視窗，既會出錯，每個視窗都缺少模型所假設的全域上下文，又會太慢，因為每個視窗都得對整段序列做數次去雜訊，把即時預算超支好幾倍。
- **它的輸出不是編碼的穩定函數。** 重疊相加之所以成立，是因為對同一段編碼重新解碼會得到（近乎）相同、可供混合的取樣。25 Hz 解碼器的輸出還取決於參考條件、擴散軌跡與全序列注意力，因此對同一段區域的兩次算繪並不會一致，而交叉淡化（正是用來藏住區塊接縫的關鍵機制）就沒有一致的東西可混合。

因此串流路徑刻意只接受 12 Hz 編解碼器；25 Hz tokenizer 則保留給離線、整段合成的路徑，讓它較重、以參考為條件的解碼器能無拘束地執行，發揮更高的保真度。
...
| DiT | Diffusion Transformer | 擴散變換器：透過對整個序列反覆去雜訊（擴散）來產生輸出，而非把輸入直接映射成輸出；25 Hz 解碼器即屬此類。 |
| x-vector | — | 從參考片段擷取、用來表徵語者音色身分的定長神經嵌入向量。 |
| BigVGAN | — | 神經聲碼器（梅爾頻譜 → 波形），是 25 Hz 解碼器的最後一級。 |
